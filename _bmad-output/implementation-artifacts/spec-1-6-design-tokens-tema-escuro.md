...
Review adversarial de 4 camadas. Intent congelado atingido (tema escuro carrega, `<head>` linka `theme.css`, tokens em `:root`, contraste AA, sem modo claro). Achados abaixo.

- [x] [Review][Patch] `outline-offset: 2px` é um literal de espaçamento fora dos tokens — viola o constraint congelado "nenhum valor de cor/tipografia/espaçamento/raio hard-coded fora das custom properties". **Decisão (D2, opção 1):** adicionar `focus-ring.offset: '2px'` ao bloco `components.focus-ring` do frontmatter de `DESIGN.md`, materializar como `--focus-ring-offset` em `:root`, trocar o literal por `var(--focus-ring-offset)`, e estender o teste de completude `(a)` pra cobrir `focus-ring.offset`. `www/theme.css:116`
- [x] [Review][Patch] Teste de integração pula em vez de falhar numa regressão de boot — `test-ui_theme.R` `(integration)`: se o `app.R` morre no boot, `proc$is_alive()` vira `FALSE`, o loop quebra, `home` fica `NULL` e `skip_if(is.null(home))` transforma em **skip**, não fail. Uma regressão que impede o app de subir — exatamente a fiação que a 1.6 introduz — passa verde. Ramificar: `fail()` (com `proc$read_all_error_lines()`) quando `!proc$is_alive()`, mantendo o skip só pro timeout real. [`tests/testthat/test-ui_theme.R:180`]
- [x] [Review][Patch] `fdwr_theme_head()` sem guarda de erro claro — `shiny::addResourcePath("fdwr-theme", "www")` lança um erro cru "directory not found" se o cwd não tem `www/` (contexto de pacote instalado, ou qualquer cwd fora da raiz). O resto do código devolve `domain_error()` estruturado. Adicionar `if (!dir.exists("www")) stop("fdwr_theme_head(): 'www' não encontrado no working directory (esperado: raiz do repo).")`. A resolução relativa em si já está em `deferred-work.md`. [`R/ui_theme.R:27`]
- [x] [Review][Defer] Guard token↔CSS `(a)` e contraste WCAG `(b)` fazem `skip_if_not(file.exists(design_path))` e `DESIGN.md` está sob `_bmad-output/` (Rbuildignored) → skip silencioso sob `R CMD check`. São os únicos guardas ligando `theme.css` a `DESIGN.md` e as únicas checagens WCAG. Rodam sob `devtools::test()` do source tree (verificação declarada). Deferido — copiar o frontmatter pra `tests/testthat/fixtures/` fecharia o gap do check empacotado.
- [x] [Review][Defer] Sem checagem reversa de token; `--focus-ring-color` não testado — teste `(a)` só cobre `DESIGN.md` → CSS. Uma custom property a mais/errada no `theme.css` só é pega pelo teste "sem hex fora do `:root`". `--focus-ring-color: var(--color-focus)` nunca é afirmado. Deferido — baixo risco, cobertura de teste.
