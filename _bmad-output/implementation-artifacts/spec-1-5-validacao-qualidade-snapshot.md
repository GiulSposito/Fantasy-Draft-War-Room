...
Review adversarial de 4 camadas (blind-hunter, edge-case-hunter, verification-gap, acceptance-auditor). ACs e I/O Matrix congelados: todos implementados e cobertos. Achados abaixo são hardening da lista coletar-tudo.

- [x] [Review][Patch] Snapshot vazio não produz bloqueante — `validate_snapshot_quality()` não replica o `snapshot_bundle_vazio` do parser. **Decisão (D1, opção 2):** documentar "o parser roda antes" como pré-condição, sem mudança de comportamento. Adicionar ao roxygen de `validate_snapshot_quality()` e à I/O Matrix congelada uma nota: a função recebe o desserializado cru e assume que `parse_snapshot_bundle()` (fail-fast, cobre `snapshot_bundle_vazio` / join / `schema_version`) roda antes no pipeline; a lista coletar-tudo é complemento, não substituto. [`R/domain_snapshot_quality.R:120`]
- [x] [Review][Patch] Tabela ilegível inunda achados — quando `players`/`metrics` não é `data.frame`, vira `data.frame()` e gera o conjunto completo de `snapshot_campo_obrigatorio_ausente` + `snapshot_opcional_ausente` + `snapshot_cobertura_anomala` em cima da causa raiz. `metadata` ilegível já suprime a cascata (`metadata_illegible`); as tabelas não. Emitir um `snapshot_tabela_ilegivel` e pular os detalhes daquela tabela. [`R/domain_snapshot_quality.R:139`]
- [x] [Review][Patch] `active_scoring_parsed` não-lista/não-erro sem guarda + `tryCatch` reusa `code` — só `is_domain_error` é tratado; um `NULL`/valor malformado cai em `verify_scoring_hash()` e sai como `snapshot_scoring_incompativel` com `details` vazio (mesmo `code` de um mismatch real, shape inconsistente pra quem consome `esperado`/`encontrado`). Espelhar o guard de `deserialized`/`metadata` (round 1): `snapshot_scoring_indisponivel` pra arg degenerado, `code` distinto pro ramo do `tryCatch`. [`R/domain_snapshot_quality.R:148`], [`R/domain_snapshot_quality.R:320`]
- [x] [Review][Patch] `qa_report$findings` como objeto JSON único é descartado em silêncio — `snapshot_quality_qa_entries()` devolve um `list(code, severity, message)` solto como está; o loop chamador itera os escalares, bate em `!is.list(entry)` e pula todos. Um achado bloqueante do qa-report não gera `qa_report_bloqueante`. Envolver um `{code,severity,message}` solto em `list(...)`. (Espelho da leitura do item de escrita já em `deferred-work.md`.) [`R/domain_snapshot_quality.R:81`]
- [x] [Review][Patch] Metadado só-espaços passa como presente — a checagem de metadado usa `!nzchar(value)` sem `trimws`, diferente dos campos de jogador (`snapshot_quality_blank` faz `trimws`). Um metadado `"   "` conta como informado. Adicionar `trimws`. [`R/domain_snapshot_quality.R:205`]
- [x] [Review][Defer] Achado `snapshot_nome_ambiguo` some quando o grupo inteiro não tem `player_id` — `sort(unique(ids[grp]))` com `ids` todos `NA` → `length(pids) >= 2L` falha, o achado não sai (as linhas já pegam `snapshot_campo_obrigatorio_ausente`). Trocar o gate para `length(grp) >= 2L`. Deferido — pré-existente, mascarado pelo bloqueante de campo obrigatório.
- [x] [Review][Defer] Regex de pureza do teste é mais estreita que a prosa — o `forbidden` bloqueia só `Sys.(time|getenv)`; `Sys.Date`, `date()`, `proc.time` passariam. Três regexes de pureza distintos (spec, teste, deferred-work). O código comprovadamente não chama nada disso; o guard é que é fraco. Deferido — pré-existente.
- [x] [Review][Defer] Dois predicados de "metadado ausente" divergem — `snapshot_validate_metadata()` (`R/domain_snapshot.R`) usa `length(value)==1 && is.na(value)`; o novo usa `all(is.na(value))`. Podem discordar em valores multi-elemento e vão derivar. Deferido — ambos corretos no caso comum; extrair helper compartilhado só se a duplicação crescer.
